...
def userdata(User_id:str):
    ...
    items_usu_games = pd.read_csv(r'df_items_userid.csv')
...
#### Voy a probar otra funcion

@app.get('/userforgenre2/{genero}')
def userforgenre2(genero:str):
    games_gen2 = pd.read_csv(r'C:\Users\juesg\Desktop\DATA SCIENCE\Proyecto_1\df_games_userforgente.csv')
    items = pd.read_csv(r"C:\Users\juesg\Desktop\DATA SCIENCE\Proyecto_1\items_userforgenre.csv")
    #def userforgenre( género : str ): Top 5 de usuarios con más horas de juego en el género dado,
    #con su URL (del user) y user_id.
    hours = items.merge(games_gen2, left_on='item_id', right_on='id') # Unimos los datasets de items y juegos
    hours = hours[hours['genres'] == genero] # Tenemos nuestros datos de nuestro genero
    total_hours = hours.groupby('user_id')['playtime_forever'].sum() # Ahora obtenemos la suma de juego por cada usuario
    top_users = total_hours.sort_values(ascending=False).head(5) # Conseguimos nuestro top 5
    top = '' # Este va a ser nuestro resultado
    for user_id, total_hour in top_users.items(): # Recorremos el top 5
        user_info = items[items['user_id'] == user_id].iloc[0] # Extraemos la informacion necesaria del usuario
        top+=f'{user_info.user_id}, {total_hours/60}, http://steamcommunity.com/id/{user_info.user_id}' # Y agregamos al resultado, el user, las horas y el url
