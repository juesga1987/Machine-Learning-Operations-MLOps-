...
def countreviews(fecha_inicial:str,fecha_final:str):
    reviews_countreviews = pd.read_csv(r'reviews.csv') #Importamos dataframe
    lista_fechas = [] 
...
def genre(genero:str):
    games_gen = pd.read_csv(r'games.csv') # Importamos dataframe
...
def sentiment_analysis(year: int):
    games_sa = pd.read_csv(r'games.csv') # Importamos Dataframe
    recomendacion_sa = pd.read_csv(r'recomendacion.csv') # Importamos Dataframe
    merged = pd.merge(recomendacion_sa, games_sa, left_on='item_id', right_on='id', how='left') #Hacemos merge con base en item_id y id
    merged = merged.drop(columns=['publisher','genres','app_name','title','url','reviews_url','price','early_access','id','developer']) #Eliminamos inncesarios
    merged.dropna(subset=['release_date_anio'], inplace=True) # eliminamos de la columna release anio nas
...
def recomendacion_juego(id:int):
    ...
    items_recomendados = modelo.iloc[indices]["id"].tolist() #Hacemos lista de Id's
